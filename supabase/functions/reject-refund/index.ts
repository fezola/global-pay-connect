...
serve(async (req) => {
  ...
  try {
    ...
    const { refund_id, rejection_reason } = await req.json();

    if (!refund_id || !rejection_reason) {
      throw new Error("refund_id and rejection_reason are required");
    }
    ...
    if (merchant.role !== "owner" && merchant.role !== "admin") {
      throw new Error("Insufficient permissions to reject refunds");
    }
